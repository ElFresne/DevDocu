...
function isNodeModuleImport(imp) {
...
function getNodeType(filePath, entity) {
    if (entity.type === 'route')
        return 'route';
    if (entity.type === 'model' || entity.name.toLowerCase().includes('model'))
        return 'model';
...
export function buildGraph(files, projectRoot) {
    const nodes = [];
    const edges = [];
    const summary = { totalFiles: 0, totalFunctions: 0, totalClasses: 0, totalRoutes: 0, totalImports: 0 };
