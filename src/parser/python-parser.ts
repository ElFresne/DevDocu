...
function extractDecorators(lines: string[], lineIndex: number): DecoratorInfo[] {
  ...
  while (i >= 0) {
    ...
    if (match) {
      decorators.push({
        ...
        args: match[3] ? match[3]!.split(',').map((a: string) => a.trim()) : [],
      })
    }
    i--
  }
...
export function parsePythonFile(filePath: string, content: string): FileNode {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const hasRouteDecorator = decorators.some(
      d => d.name === 'route' || d.name === 'app.route' || d.name === 'get' || d.name === 'post' || d.name === 'put' || d.name === 'delete' || d.name === 'patch'
    )
    ...
    if (funcMatch) {
      if (hasRouteDecorator) {
        ...
        const methodMap: Record<string, string> = { get: 'GET', post: 'POST', put: 'PUT', delete: 'DELETE', patch: 'PATCH', route: 'GET' }
